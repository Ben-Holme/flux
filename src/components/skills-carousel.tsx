...
export default function SkillsCarousel() {
  ...
  return (
    <div
      ref={ref}
      className="flex select-none gap-4 overflow-x-auto px-8 pb-6"
    ...
    >
      {SKILLS.map((skill) => (
        <div
          key={skill.title}
          className="relative flex h-80 w-56 flex-none flex-col justify-end overflow-hidden border border-white/10 bg-cover bg-center p-5"
          style={{ backgroundImage: `url(${skill.img})` }}
        >
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent" />
          <div className="relative z-10">
            <div className="mb-2 flex items-center justify-between">
              <h3 className="font-heading text-sm font-semibold tracking-wider text-parchment">
                {skill.title}
              </h3>
              <span className="text-white/60">{skill.icon}</span>
            </div>
            <p className="text-xs leading-5 text-ash">{skill.text}</p>
          </div>
          <svg
