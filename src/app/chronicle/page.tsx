...
// Normalize game coords to canvas [0,1]. Game y increases upward → flip.
function buildNorm() {
...
function drawMap(
  ...
  locations.forEach((loc, i) => {
    ...
    if (hasEvent) {
      // Gold glow for event locations
      ctx.shadowBlur = isHovered ? 16 : 8;
      ctx.shadowColor = "#ffd98f";
      ctx.fillStyle = isSelected ? "#fff" : isHovered ? "#ffe8b0" : "#ffd98f";
    } else {
      ctx.shadowBlur = 0;
    ...
    if (isHovered || isSelected) {
      ...
      ctx.textAlign = "left";
      ctx.fillStyle = hasEvent ? "#ffd98f" : "rgba(255,255,255,0.7)";
...
export default function ChroniclePage() {
  ...
  const onPointerDown = useCallback((e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // Mark as drag after a small threshold
  ...
  // Attach non-passive wheel listener so we can preventDefault and block page scroll
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
  ...
  const locEvents = selectedLoc
    ? events.filter((e) => e.location === selectedLoc.name)
    : [];
    ...
    <div style={{ position: "relative", width: "100%", height: "100vh", overflow: "hidden", background: "#0a0c0e" }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          letterSpacing: "0.2em",
          color: "#ffd98f",
          textShadow: "#ffd98f 0px 0px 6px, #ffd98f 0px 0px 12px",
        }}>
          Unyha
        </div>
        <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#ffd98f", display: "inline-block", boxShadow: "0 0 6px #ffd98f" }} />
      ...
      {selectedLoc && (
        <div style={{
          ...
          <div style={{
            ...
            textTransform: "uppercase",
            letterSpacing: "0.15em",
            color: eventLocNames.has(selectedLoc.name) ? "#ffd98f" : "rgba(255,255,255,0.85)",
            textShadow: eventLocNames.has(selectedLoc.name) ? "#ffd98f 0 0 8px" : "none",
            lineHeight: 1.3,
            marginBottom: "10px",
          }}>
            {selectedLoc.name}
